...
function multipong:init()
	menu = require "menu"	
end

function multipong:keypressed(key)
	if key == "escape" then
		paused = not paused
	end
end

function multipong:keyreleased(key)
	if (end_game or paused) and key == "return" then
		Gamestate.switch(menu)
	end
end

function multipong:focus(f)
        if not f then
                paused = true
        else
                paused = false
...
function multipong:update(delta)
	if not paused and not end_game then
		...
		if not waiting then
			...
			if love.keyboard.isDown("up") and pos2y > (raq2:getWidth()/2) * size2 then
				pos2y = pos2y - 360*delta
			elseif love.keyboard.isDown("down") and pos2y < height - (raq2:getWidth()/2) * size2 then
				pos2y = pos2y + 360*delta
			end
